(function() {
  ...
  const neededItemsData = itemsValues.map(values => {
    const assigneesCount = values[1].value.length;
    const points = values[3].value?.value || 0;
    const adjustedPoints = assigneesCount * points;
    const groupId = values[5].value?.id;
    return { adjustedPoints, groupId };
  });

  console.log({ neededItemsData });

  const groupData = columnsData.find(data => data.id === 23987125).settings.options;
